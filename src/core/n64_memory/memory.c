...
// memory sections
static u32 *readrdramreg[0xFFFF];
static u32 *readrspreg[0x30];
static u32 *readrsp[0x10];
static u32 *readmi[0x20];
static u32 *readvi[0x40];
static u32 *readai[0x20];
static u32 *readpi[0xFFFF];
static u32 *readri[0x30];
static u32 *readsi[0x20];
static u32 *readdp[0x30];
static u32 *readdps[0x20];
...
void read_rsp_mem()
{
   if (*address_low < 0x1000)
     *rdword = *((u32 *)(SP_DMEMb + (*address_low)));
   else if (*address_low < 0x2000)
     *rdword = *((u32 *)(SP_IMEMb + (*address_low&0xFFF)));
   else
     read_nomem();
}
...
void read_rsp_memd()
{
   if (*address_low < 0x1000)
     {
	*rdword = ((unsigned long long int)(*(u32 *)(SP_DMEMb + (*address_low))) << 32) |
	  ((*(u32 *)(SP_DMEMb + (*address_low) + 4)));
     }
   else if (*address_low < 0x2000)
     {
	*rdword = ((unsigned long long int)(*(u32 *)(SP_IMEMb + (*address_low&0xFFF))) << 32) |
	  ((*(u32 *)(SP_IMEMb + (*address_low&0xFFF) + 4)));
     }
   else
     read_nomemd();
}
...
{
   if (*address_low < 0x1000)
     *((u32 *)(SP_DMEMb + (*address_low))) = word;
   else if (*address_low < 0x2000)
     *((u32 *)(SP_IMEMb + (*address_low&0xFFF))) = word;
   else
     write_nomem();
}
...
void write_rsp_memd()
{
   if (*address_low < 0x1000)
     {
	*((u32 *)(SP_DMEMb + *address_low)) = dword >> 32;
	*((u32 *)(SP_DMEMb + *address_low + 4 )) = dword & 0xFFFFFFFF;
     }
   else if (*address_low < 0x2000)
     {
	*((u32 *)(SP_IMEMb + (*address_low&0xFFF))) = dword >> 32;
	*((u32 *)(SP_IMEMb + (*address_low&0xFFF) + 4 )) = dword & 0xFFFFFFFF;
     }
   else
     read_nomemd();
}
	...
	switch(ROM_HEADER->Country_code&0xFF)
	  {
	   ...
	   case 0x59:
	       {
		  u32 f = 49656530/(ai_register.ai_dacrate+1);
		  if (f)
	   ...
	   case 0x4a:
	       {
		  u32 f = 48681812/(ai_register.ai_dacrate+1);
		  if (f)
		    delay = ((unsigned long long)ai_register.ai_len*
...
void read_rom()
{
	...
	*rdword = lastwrite;
	lastwrite = 0;
     ...
     assert(rdword);
     ROMCache_read((u8*)((u32*)rdword+1), (address & 0x03FFFFFF), 4);
...
#endif
   *rdword = sl(*((u32 *)(PIF_RAMb + (address & 0x7FF) - 0x7C0)));
}
...
#endif
   *rdword = ((unsigned long long)sl(*((u32 *)(PIF_RAMb + (address & 0x7FF) - 0x7C0))) << 32)|
     sl(*((u32 *)(PIF_RAMb + ((address+4) & 0x7FF) - 0x7C0)));
}
...
#endif
   *((u32 *)(PIF_RAMb + (address & 0x7FF) - 0x7C0)) = sl(word);
...
#endif
   *((u32 *)(PIF_RAMb + (address & 0x7FF) - 0x7C0)) = 
     sl((u32)(dword >> 32));
   *((u32 *)(PIF_RAMb + (address & 0x7FF) - 0x7C0)) =
     sl((u32)(dword & 0xFFFFFFFF));
