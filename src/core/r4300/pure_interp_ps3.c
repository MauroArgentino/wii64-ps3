...
/* PS3 port notes (wii64-ps3, pure interpreter rewrite):
   - Count is advanced by the main loop (Count += 2 per instruction), NOT by
     update_count(). To keep the shared update_count() calls in
     exception_general()/TLB_refill_exception() from corrupting Count,
     pure_interpreter() sets interpcore = 2 (cached semantics) for its run.
   - Memory semantics match the cached interpreter: instruction fetch is
     hardened (RDRAM_WORDS guard, SP_DMEM, ROM_HEADER, ROMCache_read, TLB
     fallback) and load/store ops use the guarded-advance pattern below so a
     TLB/address exception keeps r4300.pc on the faulting instruction and jumps
     to the exception vector instead of advancing past it.
   - Traps (TGE/TGEU/TLT/TLTU/TNE and REGIMM) and BREAK raise
     exception_general() instead of stopping. */

#ifndef RDRAM_WORDS
#define RDRAM_WORDS 0x100000
#endif

/* After a memory access, if r4300.pc moved, the access raised an exception
   (TLB_refill_exception / exception_general set the vector pc). Mirror of
   CACHED_MEM_EXC from the cached interpreter. */
#define PS3_MEM_EXC() \
   do { \
      if (r4300.pc != ipc) return; \
   } while (0)
#define PS3_MEM_ADVANCE() \
   do { \
      if (r4300.pc == ipc) r4300.pc += 4; \
   } while (0)
...
static void JR()
{
   ...
   if (r4300.pc != ipc + 8) return;   /* delay slot raised an exception */
   r4300.delay_slot=0;
   r4300.pc = local_rs32;
...
static void MULTU()
{
   ...
   temp = (u32)rrs * (unsigned long long)((u32)rrt);
...
static void DIVU()
{
   if (rrt32)
     {
	r4300.lo = (u32)rrs32 / (u32)rrt32;
	r4300.hi = (u32)rrs32 % (u32)rrt32;
	sign_extended(r4300.lo);
	sign_extended(r4300.hi);
     }
   else printf("divu\n");
...
static void MTC0()
{
   switch(PC->f.r.nrd)
	...
	add_interrupt_event_count(COMPARE_INT, (u32)rrt);
	Compare = rrt;
	Cause = Cause & 0xFFFF7FFF; //Timer interrupt is clear
	break;
      case 12:   // Status
...
{
   set_round();
   *((s32*)r4300.fpr_single[cffd]) = *r4300.fpr_single[cffs];
   r4300.pc+=4;
}
...
{
   set_round();
   *((s32*)r4300.fpr_single[cffd]) = *r4300.fpr_double[cffs];
   r4300.pc+=4;
}
...
{
   set_rounding();
   *r4300.fpr_single[cffd] = *((s32*)r4300.fpr_single[cffs]);
   r4300.pc+=4;
}
...
{
   set_rounding();
   *r4300.fpr_double[cffd] = *((s32*)r4300.fpr_single[cffs]);
   r4300.pc+=4;
}
...
static void MFC1()
{
   rrt32 = *((s32*)r4300.fpr_single[rfs]);
   sign_extended(rrt);
   r4300.pc+=4;
}
...
static void MTC1()
{
   *((s32*)r4300.fpr_single[rfs]) = rrt32;
   r4300.pc+=4;
}
...
static void J()
{
   u32 naddr = (PC->f.j.inst_index<<2) | (r4300.pc & 0xF0000000);
   u32 ipc = r4300.pc;
   r4300.pc+=4;
   r4300.delay_slot=1;
   prefetch();
...
static void JAL()
{
   ...
   if (!r4300.skip_jump)
     {
	r4300.gpr[31]=r4300.pc;   /* pc tras el delay slot = instr+8 */
