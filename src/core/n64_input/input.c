...
{
	// Seguridad: Verificamos 'control' (0x8) y 'config' (0x10) antes de acceder
	if (Control < 0 || Control >= 4 || !virtualControllers[Control].control || !virtualControllers[Control].config)
	{
...
/**
 * Función de ayuda para el menú que devuelve los botones presionados.
 * Utilizada por WiiMenuManager en PS3.
 */
uint16_t PAD_ButtonsHeld(int Control)
{
	// Verificación defensiva completa para evitar Access Violation 0x8 y 0x10
	if (Control < 0 || Control >= 4 || !virtualControllers[Control].control || !virtualControllers[Control].config)
	{
		return 0;
	}
	...
	GetKeys(Control, &Keys); // Llama a la implementación local de GetKeys
	return (uint16_t)Keys.Value;
}
