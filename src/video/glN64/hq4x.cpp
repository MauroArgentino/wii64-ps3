/**
 * HQ4x filter — correct implementation for PS3 N64 emulator.
 * Based on HQ4x by Maxim Stepin, ported from Cameron Zemek's C implementation (LGPL-2.1).
 * Adapted for PS3: inline YUV computation (no 64MB LUT), 0xRRGGBBAA pixel format.
 */

#include "hq4x.h"
#include <stdlib.h>

static inline int hq4x_abs(int v) { return v < 0 ? -v : v; }

static inline u32 hq4x_rgb_to_yuv(u32 c) {
	int r = (c >> 24) & 0xFF;
	int g = (c >> 16) & 0xFF;
	int b = (c >>  8) & 0xFF;
	...
	return ((u32)y << 16) | ((u32)u << 8) | (u32)v;
}

#define hq4x_Ymask 0x00FF0000u
#define hq4x_Umask 0x0000FF00u
#define hq4x_Vmask 0x000000FFu
#define hq4x_trY   0x00300000u
#define hq4x_trU   0x00000700u
#define hq4x_trV   0x00000006u

static inline int hq4x_yuv_diff(u32 yuv1, u32 yuv2) {
	return (hq4x_abs((int)((yuv1 & hq4x_Ymask) - (yuv2 & hq4x_Ymask))) > (int)hq4x_trY ||
	        hq4x_abs((int)((yuv1 & hq4x_Umask) - (yuv2 & hq4x_Umask))) > (int)hq4x_trU ||
	        hq4x_abs((int)((yuv1 & hq4x_Vmask) - (yuv2 & hq4x_Vmask))) > (int)hq4x_trV);
}

#define hq4x_MASK_2     0x0000FF00u
#define hq4x_MASK_13    0x00FF00FFu
#define hq4x_MASK_ALPHA 0xFF000000u

static inline u32 hq4x_Interpolate_2(u32 c1, int w1, u32 c2, int w2, int s) {
	if (c1 == c2) return c1;
	return
		(((((c1 & hq4x_MASK_ALPHA) >> 24) * w1 + ((c2 & hq4x_MASK_ALPHA) >> 24) * w2) << (24-s)) & hq4x_MASK_ALPHA) +
		((((c1 & hq4x_MASK_2) * w1 + (c2 & hq4x_MASK_2) * w2) >> s) & hq4x_MASK_2) +
		((((c1 & hq4x_MASK_13) * w1 + (c2 & hq4x_MASK_13) * w2) >> s) & hq4x_MASK_13);
}

static inline u32 hq4x_Interpolate_3(u32 c1, int w1, u32 c2, int w2, u32 c3, int w3, int s) {
	return
		(((((c1 & hq4x_MASK_ALPHA) >> 24) * w1 + ((c2 & hq4x_MASK_ALPHA) >> 24) * w2 + ((c3 & hq4x_MASK_ALPHA) >> 24) * w3) << (24-s)) & hq4x_MASK_ALPHA) +
		((((c1 & hq4x_MASK_2) * w1 + (c2 & hq4x_MASK_2) * w2 + (c3 & hq4x_MASK_2) * w3) >> s) & hq4x_MASK_2) +
		((((c1 & hq4x_MASK_13) * w1 + (c2 & hq4x_MASK_13) * w2 + (c3 & hq4x_MASK_13) * w3) >> s) & hq4x_MASK_13);
}

#define Interp1(pc, c1, c2) *(pc) = hq4x_Interpolate_2(c1, 3, c2, 1, 2)
#define Interp2(pc, c1, c2, c3) *(pc) = hq4x_Interpolate_3(c1, 2, c2, 1, c3, 1, 2)
#define Interp3(pc, c1, c2) *(pc) = hq4x_Interpolate_2(c1, 7, c2, 1, 3)
#define Interp5(pc, c1, c2) *(pc) = hq4x_Interpolate_2(c1, 1, c2, 1, 1)
#define Interp6(pc, c1, c2, c3) *(pc) = hq4x_Interpolate_3(c1, 5, c2, 2, c3, 1, 3)
#define Interp7(pc, c1, c2, c3) *(pc) = hq4x_Interpolate_3(c1, 6, c2, 1, c3, 1, 3)
#define Interp8(pc, c1, c2) *(pc) = hq4x_Interpolate_2(c1, 5, c2, 3, 3)

#define Diff(a, b) hq4x_yuv_diff(hq4x_rgb_to_yuv(a), hq4x_rgb_to_yuv(b))
...
void hq4x_scale4x(const u32* src, u32* dst, int w, int h) {
	...
	for (j = 0; j < h; j++) {
		int prevline = (j > 0) ? -spL : 0;
		int nextline = (j < h - 1) ? spL : 0;
		const u32* sp = src + j * spL;
		u32* dp = dst + j * 4 * dpL;
		...
		for (i = 0; i < w; i++) {
			...
			int flag = 1;
			int k;
			u32 yuv1 = hq4x_rgb_to_yuv(w[5]);

			for (k = 1; k <= 9; k++) {
				if (k == 5) continue;
				if (w[k] != w[5]) {
					u32 yuv2 = hq4x_rgb_to_yuv(w[k]);
					if (hq4x_yuv_diff(yuv1, yuv2))
						pattern |= flag;
				}
				flag <<= 1;
			}
