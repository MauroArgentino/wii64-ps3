...
# include "Resource.h" // Este archivo no está en el contexto.
#else
#include <stdlib.h>
#endif // !__LINUX__
...
//#endif // !__GX__
	...
	gDP.colorImage.height = (unsigned long)(std::max( gDP.colorImage.height, (u32) gDP.scissor.lry ));
}
...
void gSPBgRect1Cyc( u32 bg )
{
	...
	if ((frameY1 - frameY0) < frameH)
	{
		...
		gDPTextureRectangle( frameX0, frameY1, frameX1 - 1, frameY2 - 1, 0, frameS0, 0, scaleW, scaleH ); // Este bloque no se utiliza.
	}
