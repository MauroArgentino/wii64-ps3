...
void RSP::executeDList()
{
   ...
   u32 *start = (u32*)(gfxInfo.RDRAM + task->data_ptr);
...
void RSP::DL()
{
   ...
   u32 addr = seg2phys(*(currentCommand+1)) & 0x7FFFFF;
   u32 *returnCommand = currentCommand;
   switch(push)
     {
      case 0: // DL_PUSH
	currentCommand = (u32*)(gfxInfo.RDRAM + addr);
